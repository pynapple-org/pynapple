...
def get_testing_set_1d():
    ...
    tc_bins = tc.index.values[None, :]
    tc = {i: tc[i] for i in tc.columns}
    ep = nap.IntervalSet(start=0, end=100)
    return feature, group, tc, tc_bins, ep


def test_decode_1d():
    feature, group, tc, tc_bins, ep = get_testing_set_1d()
    decoded, proba = nap.decode(tc, tc_bins, group, ep, bin_size=1)
...
def test_decode_1d_with_TsdFrame():
    ...
    decoded, proba = nap.decode(tc, tc_bins, count, ep, bin_size=1)
    assert isinstance(decoded, nap.Tsd)
    assert isinstance(proba, nap.TsdFrame)
    np.testing.assert_array_almost_equal(feature.values, decoded.values)
    assert len(decoded) == 100
...
def test_decode_1d_with_feature():
    feature, group, tc, tc_bins, ep = get_testing_set_1d()
    decoded, proba = nap.decode(tc, tc_bins, group, ep, bin_size=1, features=feature)
    np.testing.assert_array_almost_equal(feature.values, decoded.values)
    assert isinstance(decoded, nap.Tsd)
    assert isinstance(proba, nap.TsdFrame)
    np.testing.assert_array_almost_equal(feature.values, decoded.values)
...
def test_decode_1d_with_wrong_feature():
    ...
    with pytest.raises(RuntimeError) as e_info:
        nap.decode(tc, tc_bins, group, ep, bin_size=1, features=[1, 2, 3])
    assert str(e_info.value) == "Features should be a TsdFrame."


def test_decode_with_time_units():
    feature, group, tc, tc_bins, ep = get_testing_set_1d()
    for t, tu in zip([1, 1e3, 1e6], ["s", "ms", "us"]):
        decoded, proba = nap.decode(tc, tc_bins, group, ep, 1.0 * t, time_units=tu)
        np.testing.assert_array_almost_equal(feature.values, decoded.values)


def test_decoded_raise_errors():
    feature, group, tc, tc_bins, ep = get_testing_set_1d()
    with pytest.raises(Exception) as e_info:
        nap.decode(tc, tc_bins, np.random.rand(10), ep, 1)
    assert str(e_info.value) == "Unknown format for group"

    tc[2] = np.random.rand(2)
    with pytest.raises(Exception) as e_info:
        nap.decode(tc, tc_bins, group, ep, 1)
    assert str(e_info.value) == "Different shapes for tuning_curves and group"

    feature, group, tc, tc_bins, ep = get_testing_set_1d()
    tc = {k: values for k, values in zip(list(tc.keys())[::-1], tc.values())}
    with pytest.raises(Exception) as e_info:
        nap.decode(tc, tc_bins, group, ep, 1)
...
def get_testing_set_2d():
    features = nap.TsdFrame(
        t=np.arange(0, 100, 1),
        d=np.vstack(
            (np.repeat(np.arange(0, 2), 50), np.tile(np.arange(0, 2), 50))
        ).T.astype(np.float64),
    )
    group = nap.TsGroup(
        {
    ...
    tc, tc_bins = nap.compute_2d_tuning_curves(
        group=group, features=features, nb_bins=2, minmax=(-0.5, 1.5, -0.5, 1.5)
    )
    ep = nap.IntervalSet(start=0, end=100)
    return features, group, tc, tc_bins, ep


def test_decode_2d():
    features, group, tc, tc_bins, ep = get_testing_set_2d()
    decoded, proba = nap.decode(tc, tc_bins, group, ep, 1)
...
def test_decode_2d_with_TsdFrame():
    features, group, tc, tc_bins, ep = get_testing_set_2d()
    count = group.count(bin_size=1, ep=ep)
    decoded, proba = nap.decode(tc, tc_bins, count, ep, 1)
