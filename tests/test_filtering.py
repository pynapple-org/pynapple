...
@pytest.mark.parametrize("ep", [nap.IntervalSet(start=[0], end=[1]), nap.IntervalSet(start=[0, 0.5], end=[0.4, 1]), ])
def test_low_pass(freq, mode, order, transition_bandwidth, shape, sampling_frequency, ep):
...
@pytest.mark.parametrize("sampling_frequency", [None, 5000.0])
@pytest.mark.parametrize("ep", [nap.IntervalSet(start=[0], end=[1]), nap.IntervalSet(start=[0, 0.5], end=[0.4, 1]), ])
def test_high_pass(freq, mode, order, transition_bandwidth, shape, sampling_frequency, ep):
    t = np.linspace(0, 1, shape[0])
    y = np.squeeze(np.cos(np.pi * 2 * 80 * t).reshape(-1, *[1] * (len(shape) - 1)) + np.random.normal(size=shape))
...
@pytest.mark.parametrize("sampling_frequency", [None, 5000.0])
@pytest.mark.parametrize("ep", [nap.IntervalSet(start=[0], end=[1]), nap.IntervalSet(start=[0, 0.5], end=[0.4, 1]), ])
def test_bandpass(freq, mode, order, transition_bandwidth, shape, sampling_frequency, ep):
    t = np.linspace(0, 1, shape[0])
    y = np.squeeze(np.cos(np.pi * 2 * 80 * t).reshape(-1, *[1] * (len(shape) - 1)) + np.random.normal(size=shape))
...
@pytest.mark.parametrize("sampling_frequency", [None, 5000.0])
@pytest.mark.parametrize("ep", [nap.IntervalSet(start=[0], end=[1]), nap.IntervalSet(start=[0, 0.5], end=[0.4, 1]), ])
def test_bandstop(freq, mode, order, transition_bandwidth, shape, sampling_frequency, ep):
    t = np.linspace(0, 1, shape[0])
    y = np.squeeze(np.cos(np.pi * 2 * 80 * t).reshape(-1, *[1] * (len(shape) - 1)) + np.random.normal(size=shape))
...
@pytest.mark.parametrize("data, fs, mode, order, transition_bandwidth, expected_exception", [
    ...
    ("invalid_data", None, "butter", 4, 0.02, pytest.raises(ValueError,match="Invalid value: invalid_data. First argument should be of type Tsd, TsdFrame or TsdTensor")),
