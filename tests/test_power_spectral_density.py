...
def test_compute_power_spectral_density():
    ...
    with pytest.raises(TypeError) as e_info:
        nap.compute_power_spectral_density("a_string")
    assert (
        str(e_info.value)
...
def test_compute_mean_power_spectral_density():
    ...
    # TsdFrame
    sig2 = nap.TsdFrame(t=sig.t, d=np.repeat(sig.values[:,None], 2, 1), time_support = sig.time_support)
...
@pytest.mark.parametrize(
    ...
    (*get_signal_and_output(), 10, "a", None, False, "s", pytest.raises(TypeError, match="fs must be of type float or int")),
    (*get_signal_and_output(), 10, None, "a", False, "s", pytest.raises(TypeError, match="ep param must be a pynapple IntervalSet object, or None")),    
    (*get_signal_and_output(), 10, None, None, "a", "s", pytest.raises(TypeError, match="full_range must be of type bool or None")),
    ...
    (*get_signal_and_output(), 200, None, None, False, "s", pytest.raises(RuntimeError, match="Splitting epochs with interval_size=200 generated an empty IntervalSet. Try decreasing interval_size")),
    (*get_signal_and_output(), 10, None, nap.IntervalSet([0, 200], [100,300]), False, "s", pytest.raises(RuntimeError, match="One interval doesn't have any signal associated. Check the parameter ep or the time support if no epoch is passed.")),
    ]
)
def test_compute_mean_power_spectral_density_raise_errors(
    ...
    with expectation:
        psd = nap.compute_mean_power_spectral_density(sig, interval_size, fs, ep, full_range, time_units)
