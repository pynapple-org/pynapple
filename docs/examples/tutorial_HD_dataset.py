...
decoded, proba_feature = nap.decode_1d(tuning_curves=tuning_curves, 
                                     group=spikes_adn,                                   
                                     ep=epochs['wake'],
                                     bin_size=0.1, # second
                                     feature=position['ry'],
...
plt.imshow(np.transpose(p_feature.restrict(ep).values), aspect = 'auto', interpolation = 'bilinear', extent = [ep['start'].values[0], ep['end'].values[0],0,2*np.pi], origin='lower', cmap = 'viridis')
