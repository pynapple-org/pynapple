...
plt.bar(xtwake, cc_wake[(0,1)].values, 20, color = 'green') # The wake cross-corr of pair (0,1)
plt.title('wake')
plt.xlabel("Time (ms)")
plt.ylabel("CC")
plt.subplot(133)
# plt.plot(cc_sleep[(0,1)], color = 'red')
plt.bar(xtsleep, cc_sleep[(0,1)].values, 5, color = 'green') # The wake cross-corr of pair (0,1)
plt.title('sleep')
plt.xlabel("Time (ms)")
plt.ylabel("CC")
plt.tight_layout()
...
tuning_curves_adn = nap.compute_1d_tuning_curves(spikes_adn,position['ry'],position['ry'].time_support,nb_bins=121,minmax=(0, 2*np.pi))
