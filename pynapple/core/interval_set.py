...
class IntervalSet(pd.DataFrame):
        ...
        **kwargs
        ...
        if end is None:
            ...
            start = TimeUnits.format_timestamps(
                start.ravel(), time_units, give_warning=False
            )
            end = TimeUnits.format_timestamps(
                end.ravel(), time_units, give_warning=False
            )
        ...
        end = TimeUnits.format_timestamps(end.ravel(), time_units, give_warning=False)

        if (start[1:] == end[0:-1]).any():
            warnings.warn(
                "Some starts and ends are equal. Removing 1 microsecond!", stacklevel=2
            )
        ...
        if (start > end).any():
            warnings.warn(
                "Some ends precede the relative start. Dropping them!", stacklevel=2
            )
            idx = start < end
            start = start[idx]
            end = end[idx]
        ...
        if (start[1:] < end[0:-1]).any():
            warnings.warn(
                "Some starts precede the previous end. Joining them!", stacklevel=2
            )
            time = np.hstack((start, end))
            start_end = np.hstack(
                (
    ...
    def drop_short_intervals(self, threshold, time_units="s"):
        ...
        """
        threshold = TimeUnits.format_timestamps(
